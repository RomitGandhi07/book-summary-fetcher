...
The `home-routes` handler in the `task-wikipedia-endpoint.routes.home` namespace
...
The `home-page` function will in turn call the `task-wikipedia-endpoint.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `task-wikipedia-endpoint.handler` namespace:
...
Request middleware functions are located under the `task-wikipedia-endpoint.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `task-wikipedia-endpoint.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `task-wikipedia-endpoint.db.core` in some other namespace.
